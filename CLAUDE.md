...
- `.claude/skills/kanban-workflow/` — the `/kanban-workflow` ticket-cycle skill (`tkt-e18d0c20d6b6`). **Project-scoped**: Claude Code discovers `<projectDir>/.claude/skills` in sessions rooted here, so this copy serves kanban sessions. `SKILL.md` is tracked and must stay free of absolute paths; the project→repo map lives beside it in **gitignored** `repos.local.json`, seeded from the tracked `repos.example.json`. `repoHygiene.test.mjs` fails the suite if a home path naming a real account, or any unexpected tracked file, reaches the **index** — it does *not* check project names, which stay convention. A fresh clone gets the skill but not the map, and the skill stops and names the file rather than reporting an empty project list.
  ...
  `skillContract.test.mjs` (repo root) parses **both `SKILL.md` and this file** and fails the suite when a binding below drifts — some against an allowlist the test carries, others against a value it derives or a shape it requires. **Do not read the list as closed**: it has grown on every ticket that touched it, and the live set is the `describe` blocks in that file naming a real file rather than a fixture.
  ...
  - **§0's ticket-id argument surface** (`tkt-ec08d8af98f3`) — both spellings declared (`--ticket <id>` and the bare `tkt-[0-9a-f]{12}` shape), and the frontmatter `argument-hint` naming the flag. The hint is read from the frontmatter, never from a line-initial copy in the body, and a declaration surviving only inside a code fence does not count.
  - **§15's per-ticket-type close table** (`tkt-6dbfbd65a71c`) — its rows are derived from `TYPES` in `shared/constants.ts`, so a ticket type added there reddens the suite rather than quietly going unconsidered, and no cell carries a skip value: every type closes with the wrap-up check **asked** and the handoff **printed**. Bound to the `##` heading that *names* the wrap-up check rather than to a section number, a number being the part of a heading most likely to move.
  ...
  - **The ticket slot**, the one respect in which those two are deliberately **not** symmetric (`tkt-71229c9290b8`) — the handoff must carry a slot, since it hands the next session the id §4 ranked at close time, while the startup line must carry none, having no prior run to rank from. Checked as a required *shape*, not an allowlist: one case fails the suite if the handoff's slot is deleted, so the capability cannot quietly vanish while §15's prose still explains how to substitute it, and a second fails it if a slot is bolted onto the startup line, where an id could only ever be a guess.
