...
- `.claude/skills/kanban-workflow/` — the `/kanban-workflow` ticket-cycle skill (`tkt-e18d0c20d6b6`). **Project-scoped**: Claude Code discovers `<projectDir>/.claude/skills` only in sessions rooted here, so this copy serves kanban sessions and no others. `SKILL.md` is tracked and must stay free of absolute paths; the project→repo map lives beside it in **gitignored** `repos.local.json`, seeded from the tracked `repos.example.json`. `repoHygiene.test.mjs` fails the suite if a home path naming a real account, or any unexpected tracked file, reaches the **index** — it does *not* check project names, which stay convention. A fresh clone gets the skill but not the map, and the skill stops and names the file rather than reporting an empty project list.
  > **Interim, until `tkt-9a3afc5b9f4f` (foreign-repo mode) ships:** a second copy still lives at `~/.claude/skills/kanban-workflow/`, and it must stay. The skill's step 1 refuses a project whose repo is not the session's repo, so non-kanban work is still driven from a session in *that* repo — where only the user-scope copy exists. Deleting it now would remove that capability outright, not tidy a duplicate. The two copies have diverged (the user-scope one keeps the old hardcoded path table), and **which one wins in a kanban session is unverified** — it does not matter yet, because either serves kanban tickets, but do not assume project scope shadows user scope. Delete the user-scope copy when foreign-repo mode lands, not before
