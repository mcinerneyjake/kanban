...
2. **Delegate to the agent.** On confirmation, run:
   ...
   **sprayed** into several thin tickets. This includes *enumeration inside prose*: "three rules have
   ...
   ONE ticket for the whole report* (it used to say the opposite), and a run is hard-capped at **3
   tickets created**, so a spray is bounded and can no longer eat the step budget. Neither replaces
   the rule above: the prompt is a bias on a local model, not a guarantee, and the cap stops a
   runaway without preventing a 2–3-way split. If a run reports hitting the limit, the report was
   too broad — re-file it as separate single-issue runs rather than raising the cap.
