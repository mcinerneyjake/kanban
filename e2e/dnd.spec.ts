...
// assertion and gives a clearer message than reading `.order` off undefined.
...
test('reorder within column: ALPHA drops below BETA', async ({ page, request }) => {
  ...
  const alpha = byId(tickets, alphaId);
  const beta = byId(tickets, betaId);
...
test('midpoint math: THIRD inserts between FIRST and SECOND', async ({ page, request }) => {
  ...
  const tickets = await getTicketList(request);
  const first = byId(tickets, firstId);
  const second = byId(tickets, secondId);
  const third = byId(tickets, thirdId);
