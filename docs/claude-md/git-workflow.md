...
When the workflow runs **from inside the embedded terminal**, the session has **push + open-PR authority only by convention** — a repo-scoped GitHub PAT seeded via the container's mounted HOME (see README → *GitHub from the embedded terminal*, `tkt-fc6f493e2033`). Its Contents + Pull-requests write scopes are the same ones the merge endpoint needs, so the token does **not** stop a merge, and the ruleset requires 0 approvals (corrected 2026-09-08, `tkt-98c0377d4069`). At the merge gate the agent must therefore **not** attempt `gh pr merge`. Instead: capture the PR URL from `gh pr create` (or `gh pr view --json url -q .url`), state plainly that merging to `main` is the human's decision — the container is the least-guarded context and `guard-bash` does not run there — and print the URL for a manual merge from the host. The human-in-the-loop merge gate is deliberate; narrate it.
